...
** Extension packages
*** Gcal
    Gcal is the emacs package that connects to the Calendar API to allow for 2 ways syncing
    between specific org-mode files and GCalendar.
    #+begin_src emacs-lisp
      (use-package org-gcal
        :ensure t
        :demand t
        :custom (
                 (org-gcal-client-id (lastpass-getpass "gcal-sync-client-id"))
                 (org-gcal-client-secret (lastpass-getpass "gcal-sync-client-secret"))
                 (org-gcal-fetch-file-alist '(("qkessler@gmail.com" . "~/Documents/org_files/org-agenda/gcal-sync.org"))
                                            ))
        :config (run-with-timer 0 (* 60 60) (lambda ()
                   (interactive)
