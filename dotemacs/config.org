...
*** Theme-changer
    I'm trying out that depending on the time of the day, a.k.a sunset or sunrise, I change the theme between two options. The package (which is no longer maintained is called 'theme-changer', but it has a ton of problems. An option is using some of the code snippets found on the [[https://stackoverflow.com/questions/14760567/emacs-auto-load-color-theme-by-time][stack-overflow page]].
    #+begin_src emacs-lisp :tangle no
      (use-package solar
        :straight nil
        :init 
        ...
        :config (change-theme 'solarized-gruvbox-light 'solarized-gruvbox-dark '(37.60806276678858, -0.9865568014502395)))
#+end_src
...
**** Consult
     ...
     #+begin_src emacs-lisp
       (use-package consult
         ...
         :straight t
         :custom (consult-find-command "fdfind -color=never --full-path ARG OPTS")
...
*** Extension packages
**** Linking headings
     The Org mode manual section about internal links does refer to :CUSTOM_ID: properties for internal links. This provides you the ability to link to a heading, which then gets exported in HTML, giving the ability to create table of contents and more for org-mode files. The problem with this is that creating the org-mode headings ids is really cumbersome. @karl-voit created a few custom functions which I find interesting and thought including them in my configuration would improve my linking workflow.
     #+begin_src emacs-lisp
       (defun kv/generate-sanitized-alnum-dash-string(str)
         ...
         (when (not (org-id-get))
           (progn
             (let* (
                    ...
                    (kv/heading-text (nth 4 (org-heading-components)))
                    ;; remove progress indicators like "[2/7]" or "[25%]"
                    ;; (kv/heading-text (replace-regexp-in-string "[[][0-9%/]+[]] " "" kv/heading-text))
                    ;; get slug from heading text
                    (new-id (kv/generate-sanitized-alnum-dash-string kv/heading-text))
                    )
...
**** Org-caldav
     ...
     I also added a script I created with python3, and I find that hooking up an emacs function helps calling the script and having my tasks updated.
     #+begin_src emacs-lisp
