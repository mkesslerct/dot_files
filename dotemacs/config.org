...
>>>>>>> 0cae3b5... Adding org-download in order to get access to images from org-files
...
but default to using the "python3" binary.
#+begin_src emacs-lisp
    (use-package python
...
:warning: UPDATE 24/01/21: async server has problems atm, will untangle the file and
connect mu4e to the synchronous smtpmail-send.
#+begin_src emacs-lisp :tangle no
