...
** Enforcing commit messages
   ...
   #+begin_src emacs-lisp
     ...
     (setq conventional-commits-type-regexp (concat "^\\(" (regexp-opt
                                                            '("feat" "fix" "refactor" "docs")) "!?:\s\\)" ))
     ...
     ;; wish to enforce.
     (defcustom bw/git-commit-style-convention-checks '(summary-starts-with-capital
                                                        summary-does-not-end-with-period
                                                        summary-uses-imperative)
       "List of checks performed by `bw/git-commit-check-style-conventions'.
     ...
     (defun bw/git-commit-check-style-conventions (force)
       "Check for violations of certain basic style conventions.

          For each violation ask the user if she wants to proceed anway.
          Option `bw/git-commit-check-style-conventions' controls which
       ...
       (save-excursion
         ...
         (let ((summary (match-string 1))
           ...
           (and (or (not (memq 'summary-starts-with-capital
                               bw/git-commit-style-convention-checks))
...
** Magit
   ...
   #+begin_src emacs-lisp
     (use-package magit
       ...
       :config (add-to-list 'git-commit-finish-query-functions
                            #'bw/git-commit-check-style-conventions))
   #+end_src
* Project management
#+BEGIN_SRC emacs-lisp
