...
*** Fix evil-redirect-digit-argument
    Until upstream changes fix the evil-redirect-digit-argument function, we need to redefine it to avoid bugs in evil-org.
    #+begin_src emacs-lisp
      ;; (when (not (boundp 'evil-redirect-digit-argument))
      ;;   (defmacro evil-redirect-digit-argument (map keys target)
      ;;     `(define-key ,map ,keys ,target)))
...
*** Dark theme
     #+begin_src emacs-lisp
       (use-package doom-themes
         :straight t
         :init (add-to-list 'qk/themes-list 'doom-nord))
     #+end_src
...
*** Initial configuration
    ...
    #+begin_src emacs-lisp
      (setq org-roam-directory "~/OneDrive - Universidad Politécnica de Cartagena/slip-box")
